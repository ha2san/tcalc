...
void operand(Stack *s1, Stack *s2, Tokens tok) {
  if (!s1 || !s2)
    return;
  if (isEmpty(s2) || head(s2).type == LPARENTH ||
      (tok.type > head(s2).type && !same_level(head(s2).type, tok.type))) {
    push(s2, tok);
  } else {
    push(s1, pop(s2));
    push(s2, tok);
...
void tokens_to_postfix(List_tokens *list_token) {
  ...
  for (int i = 0; i < list_token->size; ++i) {
    ...
    case NUMBER:
      push(&output, list_token->elems[i]);
      new_size++;
      break;
    case LPARENTH:
      push(&memory, list_token->elems[i]);
      break;
    case RPARENTH:
      stack_left_parenth(&output, &memory);
      free(list_token->elems[i].value);
      break;
    default:
      operand(&output, &memory, list_token->elems[i]);
      new_size++;
    }
  }
  ...
  list_token->elems =
      realloc(list_token->elems, (size_t)new_size * sizeof(Tokens));
...
double modulo_calcul(double x, double y) {
  ...
  if (fabs(xInt - x) > 0 || fabs(yInt - y) > 0)
    printf("(The result might not be true: The modulo operator must take "
           "INTEGER as operands)\n");
  return xInt % yInt;
}
