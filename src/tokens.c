...
#ifdef DEBUG
void print_token(Tokens t) { printf("%s -> %d\n", t.value, t.type); }
...
List_tokens *get_tokens(char *input, struct hashmap *map) {
  ...
  while (until < input_length) {
    ...
    if (tokens_array[index].type != VARIABLE) {
      ...
      tokens_array[index].type = NUMBER;
    from = until;
    index++;
  }
...
size_t getUntil(size_t index, size_t *from, char *input, Tokens *t,
  ...
  case '$':
    ...
    struct mapping *m_value =
        hashmap_get(map, &(struct mapping){.variable_name = v_name});
    if (m_value == NULL) {
  ...
  case '%':
    ...
    return until;
  case '(':
    t[index].type = LPARENTH;
    return until;
  case ')':
    t[index].type = RPARENTH;
    return until;
  case '+':
    t[index].type = PLUS;
    return until;
  ...
  default:
    if (isNumber(input[*from])) {
      ...
      // might be the begining of a variable
      t[index].type = UNKNOWN;
      return until;
    }
  }
...
int isNumber(char c) { return c <= '9' && c >= '0'; }
...
void print_syntax_error(int error) {
  ...
  case ERR_SYNTAX:
    break;
  case ERR_EMPTY_PARENTHESIS:
    fprintf(stderr, "Parenthesis \")(\"\n");
    break;
  case ERR_LEFT_BEFORE_NUMBER:
    fprintf(stderr, "Lefth parenthesis before a number\n");
    break;
  case ERR_BAD_START:
    fprintf(stderr, "Bad start\n");
    break;
  case ERR_NUMBER_AFTER_NUMBER:
    fprintf(stderr, "Number after another number\n");
    break;
